...
กระบวนการเจรจาแบบมีโครงสร้างนี้ เมื่อนำมาใช้กับการออกแบบสถาปัตยกรรมซอฟต์แวร์ มีชื่อเรียกอย่างเป็นทางการว่า **ATAM (Architecture Tradeoff Analysis Method)** — วิธีการที่พัฒนาโดย Software Engineering Institute เพื่อช่วยทีมสถาปัตยกรรมให้เหตุผลเรื่อง trade-off อย่างเป็นระบบ แทนที่จะเถียงกันด้วยความรู้สึกว่า "แบบนี้น่าจะดีกว่า"
...
จากตารางนี้จะเห็น **tradeoff point** ชัดเจน: ทุกทางเลือกที่เพิ่ม consistency ให้สูงขึ้น (A และ C) ล้วนแลกกับ availability หรือ throughput ที่ลดลงในช่วง spike สูงสุด — กระทบสอง quality attribute พร้อมกัน (consistency ขึ้น availability ลง) นี่คือลักษณะของ tradeoff point ตามที่นิยามไว้ข้างต้น ต่างจาก sensitivity point ที่กระทบแค่ attribute เดียว นี่ไม่ใช่ข้อบกพร่องของการออกแบบ แต่เป็นธรรมชาติของปัญหา — เมื่อทีมเห็นจุดนี้ชัดแล้ว การตัดสินใจจึงไม่ใช่การหา "คำตอบที่ถูกต้องที่สุด" แต่คือการเลือกจุดที่รับได้มากที่สุดตาม business driver ที่ตกลงกันไว้ตั้งแต่ขั้นที่ 1

## ทำไมต้องทำเป็นกระบวนการ ไม่ใช่แค่คุยกันในที่ประชุม

ข้อดีของการทำ ATAM แบบมีโครงสร้างคือมันบังคับให้ทีมพูดถึง trade-off **ก่อน**เขียนโค้ดสักบรรทัด แทนที่จะไปเจอปัญหาตอน production แล้วค่อยถกเถียงกันว่าใครควรรู้เรื่องนี้ตั้งแต่แรก มันยังสร้างเอกสารที่ตรวจสอบย้อนหลังได้ว่าทำไมทีมถึงเลือกทางนี้ ไม่ใช่ทางอื่น ซึ่งมีประโยชน์มากเมื่อทีมใหม่เข้ามาแล้วสงสัยว่า "ทำไมไม่ใช้วิธีง่ายกว่านี้" — คำตอบอยู่ใน scenario และการวิเคราะห์ที่บันทึกไว้แล้ว ไม่ต้องเดาใหม่
