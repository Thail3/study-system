Index ในโมดูล Database (B-Tree) ช่วยหา "แถวที่ email ตรงกันเป๊ะ" ได้เร็ว เหมือนสารบัญท้ายเล่มที่บอกว่า "หัวข้อนี้อยู่หน้าไหน" — แต่ถ้าอยากค้นหาแบบ **full-text search** ("หาบทความที่มีคำว่า 'ระบบ' และ 'scale' อยู่ในเนื้อหา") สารบัญแบบเดิมช่วยไม่ได้ ต้องใช้โครงสร้างที่ต่างออกไป: **Inverted Index**
...
ลองนึกภาพสารบัญท้ายเล่มปกติ ที่บอกว่า "หน้า 850 พูดเรื่องอะไร" — แต่ inverted index ทำตรงกันข้าม มันคือ**ดัชนีคำศัพท์ท้ายเล่ม** ที่บอกว่า "คำว่า 'ระบบนิเวศ' ปรากฏอยู่หน้าไหนบ้าง (850, 902, 1050)" ชื่อ "inverted" มาจากการสลับทิศทางนี้เอง: ปกติเอกสารชี้ไปหาคำที่มันมี (doc → words) แต่ inverted index เก็บ**คำเป็นตัวหลัก แล้วชี้กลับไปว่าเอกสารไหนมีคำนั้นบ้าง** (word → docs) — ทำให้ query "หาเอกสารที่มีคำว่า X" เร็วมาก แค่ดู list ที่ผูกกับคำ X โดยตรง ไม่ต้องไล่อ่านทุกเอกสาร

## ทำไมไม่ใช้ Database ธรรมดาค้นหา

`WHERE content LIKE '%scale%'` ใน SQL ทำงานได้ แต่**ช้ามาก**กับข้อมูลจำนวนมาก (ต้องไล่อ่านทุกแถวทุกครั้ง เพราะ B-Tree index ช่วยแค่ค่าที่ตรงกันเป๊ะ ไม่ช่วยเรื่อง "มีคำนี้อยู่ตรงไหนก็ได้ในข้อความยาวๆ") นอกจากนี้ full-text search ที่ดียังต้องมี:
