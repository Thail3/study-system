...
เวลาพิมพ์ `google.com` ใน browser ก็เจอปัญหาเดียวกันเป๊ะ — เครื่องคอมพิวเตอร์ไม่รู้ **ที่อยู่จริง** ของ google.com (ซึ่งก็คือตัวเลขที่เรียกว่า **IP (Internet Protocol) address**) รู้แค่ชื่อ กระบวนการ "ถามไปเรื่อยๆ จนกว่าจะเจอที่อยู่จริง" นี้เรียกว่า <mark class="hl-term">DNS (Domain Name System) Resolution</mark>
...
ฟังดูใช้เวลานาน แต่จริงๆ ทั้งหมดนี้เกิดขึ้นในเสี้ยววินาที และ<mark class="hl-insight">เพราะมีการ "จดที่อยู่ไว้ในสมุด" (cache) ทุกขั้นตอน ครั้งต่อไปที่มีคนถามชื่อเดียวกัน แทบไม่ต้องเดินขั้นตอนซ้ำเลย</mark>

## ทำไมเรื่องนี้สำคัญกับ system design

- <mark class="hl-term">TTL (Time To Live)</mark> ก็เหมือน "อายุของที่จดในสมุด" ก่อนต้องถามใหม่ — สมมติ Google ตั้ง TTL ไว้ 300 วินาที (5 นาที) แปลว่าคำตอบที่จดไว้ใช้ได้แค่ 5 นาที พอครบเวลา resolver ต้องทิ้งของเก่าแล้วไปถามใหม่ทุกครั้งที่มีคนถามชื่อนี้อีก
  ...
  - **TTL ยาว** (เช่น 86400 วินาที = 1 วัน): resolver แทบไม่ต้องถามซ้ำเลยทั้งวัน (โหลดน้อยมาก) <mark class="hl-warning">แต่ถ้า Google ย้ายที่อยู่ระหว่างวันนั้น คนที่จดที่อยู่เก่าไว้จะยังพยายามต่อที่อยู่เก่าไปเรื่อยๆ จนกว่า TTL จะหมดอายุจริง — รู้ที่อยู่ใหม่ช้ากว่ามาก</mark>
