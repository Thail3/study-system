...
พอแตกเป็นฟู้ดคอร์ท (microservices, บทที่แล้ว) แต่ละร้านต้องคุยกันผ่าน "ทางเดินกลาง" (network) — สองมาตรฐานหลักที่ใช้กำหนด "จะคุยกันด้วยรูปแบบไหน" คือ <mark class="hl-term">REST (Representational State Transfer)</mark> (เหมือนจดหมายเต็มรูปแบบ) กับ <mark class="hl-term">gRPC (gRPC Remote Procedure Call)</mark> (เหมือนรหัสลับที่ตกลงกันไว้ก่อน)
...
props: {"nodes":[{"icon":"person","label":"External Client"},{"icon":"building","label":"Public API (REST)"},{"icon":"building","label":"Internal Service (gRPC)"}],"travelerIcon":"envelope","steps":[{"activeNode":1,"caption":"Client ภายนอกยิง REST/JSON — อ่านง่าย ไม่ต้องรู้ schema ล่วงหน้า"},{"activeNode":2,"caption":"Service ภายในคุยกันเองด้วย gRPC — มี .proto ตกลงกันไว้แล้ว เร็วกว่าเพราะเป็น binary"},{"activeNode":0,"caption":"ผลลัพธ์สุดท้ายกลับไปหา Client ภายนอก — REST หน้าบ้าน, gRPC หลังบ้าน คนละงาน คนละจุดแข็ง"}]}
```

<mark class="hl-insight">Public API ที่ third-party ต้องเรียกใช้ REST เหมาะกว่าเพราะเข้าถึงง่าย</mark> เหมือนเขียนจดหมายที่ใครก็อ่านออก ไม่ต้องมานั่งเรียนรหัสก่อน — แต่การสื่อสารภายในระหว่าง service ที่ทีมเดียวกันคุมทั้งสองฝั่ง (รู้รหัสลับตรงกันอยู่แล้ว) gRPC ให้ performance ที่ดีกว่าชัดเจน โดยเฉพาะเมื่อ traffic ภายในมีปริมาณสูงมาก (ทุกข้อความประหยัดเวลานิดหน่อย คูณด้วยจำนวนครั้งมหาศาล = ผลต่างใหญ่)
