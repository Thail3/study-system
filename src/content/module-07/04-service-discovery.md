...
Gateway (บทที่แล้ว) ต้องรู้ว่า "Order Service ตอนนี้อยู่ที่ IP ไหน" — แต่ใน microservices ที่ scale out/in ตลอดเวลา (container ถูกสร้าง/ทำลายเป็นระยะ เหมือนพนักงานย้ายที่นั่งตลอด) IP ของแต่ละ instance **เปลี่ยนตลอดเวลา** จะจำ (hardcode) IP ไว้ไม่ได้เลย **Service Discovery** คือสมุดโทรศัพท์กลางที่แก้ปัญหานี้
...
> เชื่อมกับโมดูล Scalability: Service Discovery คือ "Load Balancer เวอร์ชันที่รู้จักเฉพาะ **service ภายใน** ที่จำนวน instance เปลี่ยนตลอดเวลา" ต่างจาก Load Balancer หน้าระบบที่มักจำนวน server ค่อนข้างคงที่กว่า
