...
ระบบซอฟต์แวร์ที่มีผู้ใช้ทั่วโลกก็เจอปัญหาเดียวกัน ถ้า data center มีอยู่แค่ที่เดียว (เช่น สิงคโปร์) ผู้ใช้ที่อยู่ยุโรปหรืออเมริกาจะเจอ latency สูงมาก เพราะข้อมูลต้องเดินทางข้ามทวีปทุกครั้ง แถมถ้า region นั้นทั้ง region ล่ม (ไฟดับทั้งศูนย์ข้อมูล, ภัยธรรมชาติ) ระบบก็ล่มทั้งหมดพร้อมกัน นี่คือเหตุผลที่ระบบใหญ่ต้องทำ <mark class="hl-term">**Multi-Region Deployment**</mark>
...
<mark class="hl-term">**Active-Passive**</mark> — มี region หลัก (active) รับ traffic จริงทั้งหมด ส่วน region สำรอง (passive) ยืนรอเฉยๆ ไม่รับ traffic จนกว่า region หลักจะล่ม ค่อยสลับไปใช้ region สำรองแทน (คล้าย standby ในบท Failover & Redundancy แต่ระดับ region) ออกแบบง่ายกว่า แต่ region สำรอง "นอนเฉยๆ ไม่ได้ใช้ประโยชน์" ตอนไม่มีปัญหา

<mark class="hl-term">**Active-Active**</mark> — ทุก region รับ traffic พร้อมกันจริงๆ ตลอดเวลา ผู้ใช้แต่ละคนถูก route ไปยัง region ที่ใกล้ที่สุด ถ้า region ใดล่ม region ที่เหลือรับภาระต่อได้ทันทีโดยไม่ต้อง "สลับ" อะไร ใช้ทรัพยากรคุ้มค่ากว่าเพราะทุก region ทำงานจริง แต่ซับซ้อนกว่ามากในเรื่องการ sync ข้อมูลระหว่าง region
...
ถ้าต้องการ**ความสดใหม่ (consistency) แน่นอน** ทุกครั้งที่เขียนต้องรอ region อื่นยืนยันก่อน (latency สูงขึ้นมาก ข้ามทวีปเป็นสิบ ms ถึงร้อย ms) ถ้ายอมรับ**ความล่าช้าของข้อมูลชั่วคราว** (eventual consistency) จะได้ latency ต่ำกว่ามาก แต่ผู้ใช้ต่าง region อาจเห็นข้อมูลไม่ตรงกันชั่วขณะ — <mark class="hl-insight">นี่คือ trade-off เดียวกับ PACELC ที่เจอมาแล้ว (ไม่ใช่ CAP theorem ตรงๆ เพราะไม่มี partition) เพียงแค่มาเจอในบริบทของระยะทางข้ามทวีปจริงๆ</mark>
...
คำถามที่เจอบ่อยคือ "ระบบทุกตัวควรทำ Active-Active ทั่วโลกไหม" คำตอบตรงไปตรงมาคือ<mark class="hl-warning">**ไม่ใช่เสมอไป** — Active-Active แลกความซับซ้อนของการจัดการ consistency ข้ามทวีปมาด้วย ซึ่งมีต้นทุนสูงทั้งด้าน engineering และ operational</mark> ระบบส่วนใหญ่ที่ผู้ใช้กระจุกตัวอยู่ภูมิภาคเดียวและยอมรับ downtime สั้นๆ ได้ มักเลือก Active-Passive ที่ง่ายกว่าและเพียงพอ ส่วน Active-Active จะคุ้มค่าจริงๆ เมื่อผู้ใช้กระจายทั่วโลกจริงและ downtime มีค่าเสียหายมหาศาลจนต้องแลกความซับซ้อนมาด้วย
