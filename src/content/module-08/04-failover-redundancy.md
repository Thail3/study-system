...
Rate Limiting, Circuit Breaker, Retry (บทที่ผ่านมา) ล้วนช่วยให้ระบบ**ทนต่อความล้มเหลวชั่วคราว** — แต่ถ้า component ทั้งตัวตายจริงๆ (เครื่องพัง, data center ไฟดับ) ต้องมี**ของสำรอง**อยู่แล้ว เหมือนยางอะไหล่ในรถ นี่คือหัวใจของ <mark class="hl-term">Redundancy</mark> และ <mark class="hl-term">Failover</mark>
...
หลักการเดียวกันวนซ้ำทุกชั้น: <mark class="hl-insight">ไม่มีจุดไหนควรมีแค่ 1 ชุด</mark> ถ้ามี ต้องมี "ยางอะไหล่" พร้อมสับเปลี่ยนได้เสมอ
...
> คำถามสัมภาษณ์: "จะออกแบบระบบให้ไม่มี downtime เลยได้ไหม" — คำตอบตรงไปตรงมาคือ <mark class="hl-warning">ไม่มีระบบไหน 100% ไม่มี downtime</mark> เหมือนไม่มีรถคันไหนยางไม่แตกตลอดชีวิต เป้าหมายจริงคือลด**โอกาส**และ**ความเสียหาย**ให้ต่ำที่สุดเท่าที่งบประมาณ/ความสำคัญของระบบนั้นสมเหตุสมผล (วัดเป็น "9 กี่ตัว" เช่น 99.9% = ~8.7 ชม./ปี, 99.99% = ~52 นาที/ปี)
