...
Bounded Context คือขอบเขตที่ชัดเจน (explicit boundary) รอบๆ โมเดลหนึ่งชุด ภายในขอบเขตนั้นคำศัพท์ทุกคำมีความหมายเดียวเท่านั้น ไม่มีการตีความสองแบบ ภายใน Sales Context คำว่า `Product` หมายถึงสิ่งที่ขายได้พร้อมราคาและโปรโมชั่นเสมอ ภายใน Warehouse Context คำว่า `Product` หมายถึงหน่วยที่ต้องจัดเก็บและนับสต็อกเสมอ ทั้งสอง context มี class ชื่อ `Product` ของตัวเอง คนละ schema คนละ database ก็ได้ ไม่ต้องพยายามรวมเป็นตัวเดียวกัน
...
Bounded Context มักถูกใช้เป็นหน่วยในการตัดแบ่ง microservice เพราะถ้าตัดผิดขอบเขต (เช่น ตัดตาม database table แทนที่จะตัดตามความหมายทางธุรกิจ) จะได้ service ที่ต้องเรียกกันไปมาตลอดเวลาจนแทบไม่ต่างจาก monolith ที่ถูกแยกไฟล์เฉยๆ การเข้าใจ Bounded Context ให้ถูกต้องตั้งแต่แรกจึงเป็นรากฐานของการออกแบบระบบขนาดใหญ่ทั้งหมด
