...
ปัญหานี้ดูเล็กแต่กัดกร่อนทีมไปเรื่อยๆ ทุกครั้งที่มีคนใหม่เข้าทีมต้องมีคนคอย "แปล" ว่าคำที่ธุรกิจใช้ตรงกับโค้ดตรงไหน ทุกครั้งที่แก้ business rule ต้องนั่งเทียบว่า status 4 ใน production เปลี่ยนพฤติกรรมยังไง และที่แย่ที่สุดคือ bug มักเกิดจากช่องว่างตรงนี้เอง — dev คนหนึ่งเข้าใจว่า status 4 คือ "ยกเลิกโดยลูกค้า" แต่จริงๆ ทีมอื่นใช้เลขเดียวกันหมายถึง "ยกเลิกโดยระบบเพราะสต็อกหมด" ความหมายเพี้ยนไปทีละนิดจนกลายเป็น bug ที่ยากจะสืบ

DDD เรียกช่องว่างนี้ว่าปัญหาของ "translation layer" ระหว่างภาษาที่ธุรกิจใช้คุยกันกับภาษาที่โค้ดใช้เขียน วิธีแก้คือทำให้สองภาษานี้เป็นภาษาเดียวกันไปเลย นั่นคือแนวคิด **Ubiquitous Language** — คำศัพท์ชุดเดียวที่ใช้เหมือนกันทุกที่: ในบทสนทนากับ domain expert, ในเอกสาร, และในโค้ดจริง ไม่มีขั้นตอนแปลระหว่างกลาง
...
อีกจุดที่เชื่อมกับหัวข้อก่อนหน้าคือ Ubiquitous Language นั้นใช้ได้เฉพาะภายใน **Bounded Context** เดียวเท่านั้น คำว่า "ออเดอร์" ใน Sales Context กับ "ออเดอร์" ใน Shipping Context อาจมีรายละเอียดต่างกัน — Sales สนใจราคาและส่วนลด Shipping สนใจที่อยู่จัดส่งและ tracking number แต่ละ context มีภาษาของตัวเองที่สอดคล้องกันภายใน ไม่จำเป็นต้องเหมือนกันข้าม context
...
หลายคนมองว่า Ubiquitous Language เป็นแค่เรื่อง naming convention แต่จริงๆ แล้วมันคือเครื่องมือสื่อสารระหว่างคนสองกลุ่มที่คิดคนละแบบ — domain expert ไม่สนใจ implementation detail และ dev ไม่รู้ business context ลึกพอ ถ้าทั้งคู่ใช้คำเดียวกัน การประชุมออกแบบ feature ใหม่จะเร็วขึ้นมาก เพราะไม่ต้องเสียเวลาแปลไปแปลมา และเวลาอ่าน code review หรือ commit message ก็จะเข้าใจ business intent ได้ทันทีโดยไม่ต้องถามใคร
