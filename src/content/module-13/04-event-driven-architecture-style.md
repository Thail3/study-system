...
ไดอะแกรมด้านบนคือ **Choreography** — เหมือนวงแจ๊ส ไม่มี service ไหนสั่งใคร Order Service แค่ประกาศว่า "order ถูกสร้างแล้ว" ผ่าน event bus ส่วน Payment, Inventory, Shipping, Notification ต่างฟังและตัดสินใจทำงานของตัวเองอย่างอิสระ ไม่มี service ไหนรู้จัก flow ทั้งหมดตั้งแต่ต้นจนจบเลยด้วยซ้ำ ตรงข้ามกับ Choreography คือ **Orchestration** — เหมือนวงออร์เคสตรา มี service กลางหนึ่งตัว (มักเรียกว่า Saga Orchestrator) ทำหน้าที่เป็นวาทยกร คอยเรียก Payment Service ก่อน รอผลลัพธ์ แล้วค่อยเรียก Inventory Service ต่อ รู้จักและควบคุม flow ทั้งหมดจากจุดเดียว รวมถึงตัดสินใจว่าถ้าขั้นตอนไหนล้มเหลว ต้อง compensate (ย้อนกลับ) ยังไง

## ผลต่อ topology และขอบเขตของทีม

การเลือกระหว่างสองแบบนี้ไม่ใช่แค่รายละเอียดทางเทคนิค แต่เปลี่ยนหน้าตาขององค์กรและระบบไปพร้อมกัน ใน Choreography ไม่มี service ไหนรู้จัก flow ทั้งหมด แต่ละทีมเป็นเจ้าของ service ของตัวเองเต็มที่ เพิ่มหรือแก้ logic ในทีมตัวเองได้โดยไม่ต้องขอทีมอื่น (decentralized ownership) แต่ต้องแลกกับการ debug ที่ยากขึ้นมาก เพราะไม่มีที่เดียวให้ดู flow ทั้งหมด ต้องพึ่ง distributed tracing เพื่อตามรอย ส่วนใน Orchestration มีทีมเดียวเป็นเจ้าของ orchestrator ต้องเข้าใจและแก้ไขทุก step เวลาต้องการเปลี่ยน flow ซึ่งถ้าออกแบบไม่ดีอาจกลายเป็นคอขวดคล้ายกับ ESB ในสถาปัตยกรรม SOA แต่แลกมาด้วยการมองเห็น flow ทั้งหมดได้จากจุดเดียว ทำให้ debug และ reason เกี่ยวกับระบบง่ายกว่ามาก
...
ในการสัมภาษณ์งาน คำถามแนวนี้มักถามต่อว่า "ถ้า flow ซับซ้อนขึ้นเรื่อยๆ จะยังใช้ choreography ต่อไหม" คำตอบที่ดีคือชี้ให้เห็นว่าทั้งสองแบบไม่ใช่ทางเลือกตายตัว ระบบใหญ่จำนวนมากเริ่มจาก choreography ในส่วนที่ง่าย แล้วค่อยดึงบาง flow ที่ซับซ้อนมาก (มี compensating transaction หลายขั้น) ออกมาทำเป็น orchestration แยกต่างหาก
