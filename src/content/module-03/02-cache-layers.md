...
3. **Server-side / In-memory Cache (ตู้เย็นในบ้าน)** — เก็บอยู่ใน memory ของ process server เอง เร็วมาก แต่ปัญหาคือ**ไม่แชร์กับบ้านอื่น** (server เครื่องอื่น) — คล้ายปัญหา sticky session ที่เจอในโมดูล Scalability
4. **Distributed Cache: Redis (ร้านสะดวกซื้อกลางเมืองที่ทุกบ้านไปซื้อได้)** — cache กลางที่ทุก server เข้าถึงร่วมกันได้ แก้ปัญหาข้อ 3 แต่ต้องเดินทาง (network call) เพิ่มขึ้นนิดหน่อย
