...
ส่วนฝั่ง **read** มี process แยกต่างหากเรียกว่า <mark class="hl-term">**Projector**</mark> (บางที่เรียก Event Handler หรือ Subscriber) คอย **subscribe** ฟัง event ใหม่ๆ ที่ถูก append เข้า Event Store แล้วนำมาสร้างหรืออัปเดต **Read Model / Projection** — ตารางหรือ view ที่ denormalize พร้อมแสดงผลทันที Query ทุกอันจากฝั่งผู้ใช้จะอ่านจาก Read Model นี้เท่านั้น ไม่แตะ Event Store โดยตรง
...
ถ้าเป็นระบบที่เก็บแค่ current state (ไม่มี event sourcing) การสร้าง read model รูปแบบใหม่ย้อนหลังแบบนี้แทบเป็นไปไม่ได้ <mark class="hl-warning">เพราะข้อมูลในอดีตถูกเขียนทับหายไปแล้ว รู้ได้แค่ "ตอนนี้เป็นยังไง" ไม่รู้ "อดีตเคยเป็นยังไง"</mark> — นี่คือเหตุผลที่ CQRS กับ Event Sourcing มักถูกใช้ร่วมกันในระบบที่คาดว่าความต้องการด้าน reporting/analytics จะเปลี่ยนแปลงบ่อยในอนาคต
