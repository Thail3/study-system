...
ตาราง `users` ในฐานข้อมูลที่มี 10 ล้านแถวก็เจอปัญหาเดียวกัน — query `WHERE email = 'x@example.com'` ถ้าไม่มี index ต้องไล่อ่านทีละแถวจนกว่าจะเจอ (**full table scan** — เหมือนพลิกหนังสือทีละหน้า) ช้ามาก **Index** คือดัชนีท้ายเล่มของฐานข้อมูล — โครงสร้างเสริมที่บอกว่า "ข้อมูลที่ต้องการอยู่ตรงไหน" โดยไม่ต้องไล่ทั้งตาราง
...
- **อ่านเร็วขึ้นมาก** แต่ **เขียนช้าลง** — ทุกครั้งที่ insert/update/delete ต้องอัปเดต index ด้วย ไม่ใช่แค่ตารางข้อมูลเฉยๆ
- **กิน storage เพิ่ม** — index คือโครงสร้างข้อมูลแยกต่างหาก ยิ่งมี index เยอะ ยิ่งกินพื้นที่มากขึ้น
- **เลือก column ให้ index อย่างเลือกสรร** — ไม่ใช่ทุก column ต้อง index เหมือนไม่ใช่ทุกคำในหนังสือต้องอยู่ในสารบัญ ควร index เฉพาะ column ที่ใช้ query บ่อย (โดยเฉพาะใน `WHERE`, `JOIN`, `ORDER BY`)
