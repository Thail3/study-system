...
## ขั้นสูง (สั้นๆ): เปิดสาขาใหม่ไม่ได้พร้อมขายทันทีที่เปิดไฟ

- **จะรู้ได้ยังไงว่าต้องเปิดสาขาเพิ่มเมื่อไหร่** — autoscaling ตัดสินใจจาก metric บางอย่าง: **CPU-based** (เปิดเพิ่มเมื่อ CPU เกิน threshold) ใช้ง่ายแต่สายไปสำหรับงานที่รอ I/O เป็นหลัก (CPU ไม่พุ่งทั้งที่งานค้างอยู่เพียบ) ส่วน **queue-depth-based** (เปิดเพิ่มเมื่องานค้างในคิวเยอะเกิน) สะท้อนภาระจริงของงานแบบ async ได้ตรงกว่า
- <mark class="hl-warning">สาขาใหม่ที่เพิ่งเปิด "เตายังเย็น" (cache ยังไม่มีอะไรเลย) ถ้าโยน traffic เต็มโควต้าเข้าไปทันที ทุก request แรกจะ miss cache หมดพร้อมกัน กระแทก database หนักกว่าตอนสาขาเก่าๆ ที่ cache warm แล้วเสียอีก</mark> — ทางแก้คือค่อยๆ ทยอยส่ง traffic เข้าเครื่องใหม่ (คล้าย canary rollout จากโมดูล Reliability) ให้ cache มีเวลา warm ขึ้นก่อน ไม่ใช่เปิดประตูรับลูกค้าเต็มร้านตั้งแต่วินาทีแรก
- ตอน**ปิด**สาขา (scale-in) ก็ต้องระวังแบบเดียวกัน — <mark class="hl-term">Connection Draining</mark> คือให้ Load Balancer หยุดส่งลูกค้าใหม่มาสาขานี้ก่อน แต่รอให้ลูกค้าที่กำลังกินอยู่ (request ที่ประมวลผลค้างอยู่) เสร็จก่อนค่อยปิดไฟจริง ไม่ใช่ตัดไฟกลางคันทันที
