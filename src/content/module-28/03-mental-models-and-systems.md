...
สังเกตว่าลำดับสี่ระดับนี้**ตรงกับลำดับ leverage point จากโมดูลก่อนหน้าเป๊ะ** — Events ≈ parameter (เห็นง่าย แก้ง่าย ผลน้อย), Structure ≈ rules/loop strength (ต้องขุดหาแต่ผลแรงกว่า), Mental Models ≈ paradigm (ลึกที่สุด แต่ทรงพลังที่สุด) นี่ไม่ใช่ความบังเอิญ — Iceberg Model กับ Leverage Points คือมุมมองสองมุมของกรอบคิดเดียวกัน ยิ่งขุดลึกลงไปใต้ผิวน้ำ ยิ่งเจอจุดที่แก้แล้วได้ผลกว้างและยั่งยืนกว่า
...
การแก้ที่ event ("restart server") ได้ผลแค่วันนั้น การแก้ที่ mental model ("เปลี่ยนมุมมองทีมให้ถือว่า traffic spike คือเรื่องปกติที่ต้องออกแบบรองรับไว้ตั้งแต่ต้น ไม่ใช่ edge case") จะเปลี่ยนวิธีที่ทีมออกแบบระบบทุกตัวถัดไป ไม่ใช่แค่ตัวนี้ตัวเดียว
