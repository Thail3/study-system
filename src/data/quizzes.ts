...
const RAW_QUIZZES: Record<string, QuizQA[]> = {
  ...
  'architecture-case-studies:legacy-monolith-migration': [
    { question: 'ทำไมทีมถึงเลือก Stock Lookup เป็น capability แรกที่ migrate ออกจาก legacy monolith แทนที่จะเป็น Inventory Reservation หรือ Order Fulfillment ที่มีมูลค่าทางธุรกิจสูงกว่า?', answer: 'เพราะหลักการเลือกลำดับ migrate ใน Strangler Fig คือความเสี่ยงต่ำและ boundary ชัดที่สุดก่อน ไม่ใช่ฟีเจอร์ที่ธุรกิจอยากได้เร่งด่วนที่สุด — Stock Lookup เป็น read-only ไม่มี side effect พังแล้วแค่ข้อมูลแสดงคลาดเคลื่อนชั่วคราว ต่างจาก Inventory Reservation/Order Fulfillment ที่ coupling แน่นและความเสี่ยงสูงมาก ถ้าเลือกตัวที่เสี่ยงสูงก่อนแล้วพัง ทีมจะเสียความเชื่อมั่นในแนวทาง strangler fig ทั้งหมด' },
    { question: 'Fitness Function ในเคสนี้มีไว้ป้องกันปัญหาอะไร และทำงานอย่างไร?', answer: 'มีไว้ป้องกันไม่ให้ service ใหม่ที่เพิ่ง migrate ออกมาแอบกลับไปมี coupling แบบเดิม (เช่น query database ของ monolith ตรงๆ) ซึ่งเป็นสาเหตุที่ทำให้ระบบเดิมกลายเป็นแตะไม่ได้ตั้งแต่แรก ทำงานโดยผูกกฎอัตโนมัติเข้า CI pipeline เช่น dependency check ห้าม import ข้าม service ตรง และเช็ค latency threshold ถ้า pull request ละเมิดกฎ build จะ fail ทันทีก่อน merge' },
    { question: 'อะไรคือสาเหตุรากของการที่ legacy monolith กลายเป็นระบบที่ไม่มีใครกล้าแตะ ตามที่เคสนี้อธิบายไว้?', answer: 'ไม่ใช่เพราะมี developer ที่ไม่มีวินัย แต่เป็นผลลัพธ์ที่คาดเดาได้ของระบบที่ไม่มีการตรวจสอบ boundary อัตโนมัติ ปล่อยให้ทุกอย่างพึ่งความจำและวินัยของคนล้วนๆ — deadline กระชั้นทำให้ทีมเขียน shortcut ข้าม module ตรงทีละจุดสะสมนานแปดปี (Architecture Erosion) จนกลายเป็น big ball of mud ที่ทุกจุดเชื่อมกับทุกจุด' },
  ],
  'architecture-case-studies:game-platform-at-scale': [
    { question: 'ทำไม Combat/Matchmaking ถึงควร deploy แบบ Active-Active ต่อ region ในขณะที่ Guild/Economy ต้องรวมศูนย์เป็น global service เดียว?', answer: 'เพราะลักษณะข้อมูลต่างกัน — Combat เป็นเรื่อง "เวลาจริงตรงหน้า" latency สูงแค่ร้อยกว่า ms ก็ทำให้เกม feel ผิด และแต่ละแมตช์จบในตัวเองไม่เกี่ยวกับ region อื่น จึงกระจายท้องถิ่นได้อย่างปลอดภัย ส่วน Guild/Economy เป็น "บันทึกความจริงที่ต้องใช้ร่วมกันข้ามคน" ถ้าแยกเก็บตาม region จะเจอปัญหาชื่อกิลด์ซ้ำหรือไอเทมถูกนับซ้ำ จึงต้องมี global source of truth เดียว' },
    { question: 'ในสถาปัตยกรรมนี้ ทำไม leaderboard/trading-history ถึงถูกออกแบบเป็น CQRS read side แยกจาก combat server?', answer: 'เพราะทำให้ leaderboard scale ได้อิสระจาก combat server โดยสิ้นเชิง ผู้เล่นเป็นล้านคนกดดู leaderboard พร้อมกันไม่กระทบ latency ของแมตช์ที่กำลังสู้กันสดอยู่เลย เนื่องจากสองฝั่งไม่ได้ใช้ database เดียวกัน — combat server (write model) ปล่อย event เข้า event bus แล้ว read model ค่อย subscribe มาสร้าง denormalized view ต่างหาก' },
    { question: 'ทำไมเคสนี้ถึงเตือนว่าไม่ควรแบ่งทีมละเอียดแบบนี้ตั้งแต่วันที่บริษัทยังเป็นทีมเดียวสิบกว่าคน?', answer: 'เพราะ Inverse Conway Maneuver คุ้มค่าเมื่อขอบเขต bounded context นิ่งพอแล้วเท่านั้น ถ้ายังไม่แน่ใจว่า Guild กับ Economy ควรแยกกันจริงไหม การจัดทีมแยกไปก่อนจะสร้างต้นทุนประสานงานข้ามทีมที่ไม่จำเป็น ควรรอให้สถาปัตยกรรมชัดเจนพอสมควรก่อนค่อยจัดทีมตาม' },
  ],
  'architecture-case-studies:secure-payment-platform': [
    { question: 'ทำไมการเชื่อว่า service ที่ "อยู่ใน VPN เดียวกัน" ปลอดภัยพอโดยไม่ต้องตรวจตัวตนต่อ call ถึงเป็นจุดอ่อนร้ายแรง?', answer: 'เพราะถ้า service ไหนถูกเจาะแม้จุดเดียว ผู้บุกรุกจะเรียก internal API ที่เหลือได้ทันทีเหมือนเป็น service นั้นเอง (lateral movement ได้ทั้งระบบ) — Zero Trust Architecture แก้ตรงนี้ด้วยการเปลี่ยนหน่วยความเชื่อถือจาก "อยู่ใน network ไหน" เป็น "identity ของ request นี้" บังคับ mTLS และ verify ตัวตนทุก service-to-service call' },
    { question: 'ทำไมการเพิ่ม WAF อีกตัวที่กันวิธีเดียวกันไม่ถือเป็น Defense in Depth ที่แท้จริง?', answer: 'เพราะหัวใจของ Defense in Depth คือความหลากหลายของกลไกป้องกัน ไม่ใช่จำนวนชั้น — ถ้า WAF สองตัวกันแบบเดียวกัน โดน bypass วิธีเดียวกันก็ล้มทั้งคู่พร้อมกัน ต้องมีชั้นป้องกันที่เป็นอิสระจากกันจริง เช่น Network, Identity, Application, Data, Monitoring ที่แต่ละชั้นปิดช่องโหว่คนละประเภท' },
    { question: 'STRIDE ช่วยจับช่องโหว่ทั้ง 4 ข้อที่ pentest เจอได้อย่างไร?', answer: 'STRIDE คือเช็คลิสต์ 6 หมวดที่ควรไล่คิดตอนออกแบบ แต่ทีมนี้สร้าง payment flow เสร็จก่อนแล้วค่อยคิดเรื่องความปลอดภัยทีหลัง ทำให้ทุกช่องโหว่ที่ pentest เจอ (spoofing จาก VPN trust, information disclosure จาก credential หลุด ฯลฯ) ล้วนผูกกับ trust boundary ที่ไม่เคยถูกวาดไว้เลย ถ้าทำ threat modeling ตั้งแต่ตอนออกแบบ จะจับช่องโหว่เหล่านี้ได้ตั้งแต่ต้นด้วยต้นทุนต่ำกว่ามาก' },
  ],
  'architecture-case-studies:team-reorg-for-scale': [
    { question: 'ทำไมปัญหาของบริษัทนี้ถึงถูกวินิจฉัยว่าเป็น "ทีมเดียวขนาด 40 คนที่ไม่เคยถูกแบ่ง" ไม่ใช่แค่ "คนเยอะเกินไป"?', answer: 'เพราะทุกคนยังมี mental model แบบตอนมี 5 คนคือ "ใครจะแก้ตรงไหนก็ได้" พอ codebase โตตามจำนวนคน แต่ละคนต้องถือความซับซ้อนของทั้งระบบพร้อมกัน (cognitive load เกินขีดจำกัด) นี่คือสาเหตุตรงของ PR review ช้าและการแก้ Order กระทบ Payment/Shipping โดยไม่มีใครเป็นเจ้าของ boundary จริง' },
    { question: 'ทำไมเคสนี้ถึงเลือกประกาศแบ่งทีมก่อนที่ codebase จะถูก refactor แยกเสร็จ แทนที่จะรอให้โค้ดแยกเป็น module ชัดเจนก่อนแล้วค่อยจัดทีมตาม?', answer: 'ตาม Inverse Conway Maneuver การรอให้ refactor เสร็จก่อนจะทำให้ทีมชนกันไปเรื่อยๆ ระหว่างทาง ควรกลับด้านคือจัดทีมให้ชัดก่อน เมื่อคนที่ทำงานด้วยกันทุกวันเปลี่ยนไป ขอบเขตการสื่อสารก็เปลี่ยนตาม แล้วโค้ดจะเริ่มเกาะกลุ่มตามขอบเขตทีมใหม่เองตามธรรมชาติโดยไม่ต้องมีใครสั่งแยกอย่างเป็นทางการ' },
    { question: 'ทำไมถ้า Platform team ต้องเข้าประชุม Collaboration กับทุกทีมทุกสัปดาห์เพื่อช่วย deploy ถึงถือว่าการ reorg ล้มเหลว?', answer: 'เพราะ Platform team จะกลายเป็นคอขวดใหม่แทนที่ทีมเดียวเดิม เพียงแค่เปลี่ยนชื่อคอขวด ทางเลือกที่ถูกต้องคือให้ Platform team ลงทุนสร้าง self-service tooling แบบ X-as-a-Service ให้ทีมอื่นเรียกใช้เองได้โดยแทบไม่ต้องคุยกันในงานประจำวัน' },
  ],
  'architecture-case-studies:undocumented-legacy-onboarding': [
    { question: 'ทำไมการสร้าง C4 Container diagram ของระบบไร้เอกสารถึงควรเริ่มจากการสำรวจ infra จริง (deployment config, crontab, queue consumer) แทนที่จะเริ่มจากการถามคนในทีมเดิม?', answer: 'เพราะระบบที่รันอยู่จริงโกหกไม่ได้ ต่างจากความทรงจำของคนที่คลาดเคลื่อนได้ตลอดเวลาโดยที่เจ้าตัวไม่รู้ตัว ควรใช้ของจริงเป็น source of truth หลักแล้วเอาคำบอกเล่าของคนมาช่วยยืนยันหรือเติมเจตนา/เหตุผลเบื้องหลังทีหลัง ไม่ใช่ใช้เป็นแหล่งข้อมูลหลักตั้งแต่แรก' },
